...
1. **음악 공연만 등록** — 이 플랫폼은 인디 음악/아티스트 팬 플랫폼이다. 아래는 등록하지 않는다:
   ...
   - **키워드 제외**: 야구, 축구, 농구, 배구, 스포츠, KBO, KBL, K리그 등 스포츠 + 토크콘서트, 토크쇼, 강연, 세미나, 특강 등 비-음악 이벤트 + 트로트 + 오케스트라, 필하모닉, 심포니, 교향 등 클래식/관현악 + 합창(합창단 등) + 팬미팅 (서버가 제목 키워드로 400 거부)
...
fetcher의 `artistNames`는 자주 비어있다. 아래 절차로 아티스트를 식별한다:

1. **제목 전처리**: `scripts/clean-title.sh`로 비-아티스트 패턴(`(For. X)`, 스폰서 괄호 등)을 결정적으로 제거한 뒤, 전처리된 제목으로 판단한다.
2. **`artistNames`에 값이 있으면** → 그대로 사용하되, 부제목이 섞여있으면 걸러낸다.
3. **비어있으면** → 전처리된 제목을 보고 네 지식으로 아티스트를 판단한다.
4. **페스티벌** → 가능한 만큼 식별. 확실하지 않으면 추측하지 않는다.

상세 예시와 엣지케이스는 `references/artist-identification.md` 참조. regex가 아니라 **네 지식**을 쓰는 것이 핵심이다.
...
- **제목 키워드 제외**: 야구, 축구, 농구, 배구, 스포츠, KBO, KBL, K리그 등 스포츠 + 합창 등 비-인디 음악
