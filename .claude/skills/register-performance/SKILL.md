...
6. **타임테이블 배치** (lineupMode가 TIMETABLE이면 — 아래 "타임테이블 자동 배치" 섹션 참조)
7. **상태 업데이트**: 플랫폼 상태 + 글로벌 상태 저장.
...
1. **find-or-create**: `POST /artists/find-or-create` with `{"name": "아티스트명"}`.
   ...
   - Spotify 검색 결과가 없는 경우에만 이름만으로 생성됨.
...
4. 변경 있으면 → `PATCH /performances/:id`로 업데이트. status 변경 시 반드시 `status`와 `salesStatus` 둘 다 포함.
5. **아티스트 & 타임테이블 보완 (409인 기존 공연도 반드시 체크)**:
