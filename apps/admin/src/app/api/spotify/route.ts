...
// Spotify가 자동 생성하는 페이지 메타 텍스트 감지
function isAutoGeneratedDescription(desc: string): boolean {
  const lower = desc.toLowerCase();
  return (
    /monthly listeners?/i.test(lower) ||
    /where people listen/i.test(lower) ||
    /^artist\s*·/i.test(desc.trim()) ||
    /·\s*song:/i.test(desc)
  );
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // JSON-LD에서 description 추출 (자동 생성된 메타 텍스트는 필터링)
    ...
    if (jsonLdMatch) {
      try {
        ...
        const raw = jsonLd.description || null;
        if (raw && !isAutoGeneratedDescription(raw)) {
          description = raw;
        }
      } catch {}
    }
